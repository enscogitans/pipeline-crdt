...
from yaml_diff_v3.crdt_graph.nodes import NodeId, Timestamp
...
def _convert_edit_scalar_node(yaml_update: yaml.EditScalarNode, session_id: SessionId, ts: Timestamp,
                              path_to_id_mapping: dict[yaml.NodePath, NodeId]) -> EditScalarNode:
    return EditScalarNode(
        session_id=session_id,
        timestamp=ts,
        update_id=UpdateId(make_unique_id()),
        node_id=NodeId(path_to_id_mapping[yaml_update.path]),
...
def _convert_add_map_item(yaml_update: yaml.AddMapItem, session_id: SessionId, ts: Timestamp,
                          path_to_id_mapping: dict[yaml.NodePath, NodeId]) -> AddMapItem:
    new_item = make_new_crdt_mapping_item_from_yaml(yaml_update.new_item, ts)
    return AddMapItem(
        session_id=session_id,
        timestamp=ts,
        update_id=UpdateId(make_unique_id()),
        mapping_node_id=path_to_id_mapping[yaml_update.map_path],
        new_item=new_item,
    )


def _convert_delete_map_item(yaml_update: yaml.DeleteMapItem, session_id: SessionId, ts: Timestamp,
                             path_to_id_mapping: dict[yaml.NodePath, NodeId]) -> DeleteMapItem:
    return DeleteMapItem(
        session_id=session_id,
        timestamp=ts,
        update_id=UpdateId(make_unique_id()),
        item_value_id=path_to_id_mapping[yaml_update.path],
    )
...
def make_crdt_updates_from_yaml_updates(yaml_updates: list[yaml.Update], session_id: SessionId,
    ...
    result = []
    path_to_id_mapping = graph.get_path_to_id_mapping()
    for yaml_update in yaml_updates:
        converted: Update
        if isinstance(yaml_update, yaml.EditScalarNode):
            converted = _convert_edit_scalar_node(yaml_update, session_id, ts, path_to_id_mapping)
        elif isinstance(yaml_update, yaml.AddMapItem):
            converted = _convert_add_map_item(yaml_update, session_id, ts, path_to_id_mapping)
        elif isinstance(yaml_update, yaml.DeleteMapItem):
            converted = _convert_delete_map_item(yaml_update, session_id, ts, path_to_id_mapping)
        else:
