...
def test_add_map_item():
    check("A: 1", "A: 1\nB: 2",
          [AddMapItem(map_path=(), new_item=MappingNode.Item(
              path_key="B",
              key=ScalarNode(path=("B", 0), tag=get_tag("str"), value="B"),
              value=ScalarNode(path=("B", 1), tag=get_tag("int"), value="2"),
          ))])
    ...
    new_item = MappingNode.Item(
        path_key="A",
        key=ScalarNode(path=(2, "A", 0), tag=get_tag("str"), value="A"),
        value=MappingNode(path=(2, "A", 1), tag=get_tag("map"), items=OrderedDict({
            "X": MappingNode.Item(
                path_key="X",
                key=ScalarNode(path=(2, "A", 1, "X", 0), tag=get_tag("str"), value="X"),
                value=ScalarNode(path=(2, "A", 1, "X", 1), tag=get_tag("str"), value="x"),
            )
        }))
    )
...
def test_del_map_item():
    check("A: 1\nB: {C: {D: d}}", "A: 1", [DeleteMapItem(path=("B",))])
    check("A: 1\nB: 2\nC: {D: {E: e, F: f}}",
          "A: 1\nC: {D: {E: e}}",
          [DeleteMapItem(path=("B",)), DeleteMapItem(path=("C", 1, "D", 1, "F"))])
